   ...
   className={`z-3 relative transition-all duration-1000`} />
    <div
        className={`fixed inset-0 bg-gradient-to-b from-black/70 to-black/70 
          z-5 transition-opacity duration-300 md:hidden ${
      ...
      >
         {modal &&(
              ...
              onConfirm={handleLogout} onCancel={()=>setModal(false)} onOpen={()=>setModal(true)} />
             )}
            

        <div
          className={`fixed left-0 top-0 h-full w-64 bg-cyan-600 shadow-xl transform 
        ...
        >
            ...
            <nav>
              <ul
          className="slidein mt-20 flex flex-col px-5 space-y-5 pt-6 justify-center">
              {navLinks.map((item) => 
                <li
                  key={item.id}
                   className=" bg-black/70 rounded-md p-2 
                  transition-all duration-300 hover:bg-black/50 hover:pl-5  ">
                <Link 
                href={item.href}
                  onClick={()=> setOpen((false))}
               ...
               <button  onClick={()=> setModal(true)}
                variant="primary" className={`text-left text-xl bg-black/70 rounded-md p-2 
                  transition-all duration-300 hover:bg-red-900 hover:text-cyan-500 hover:pl-5 ${textColor}`} >Logout</button>
    ...
    <nav>
      ...
      {navLinks.map((item)=> 
      <li key={item.id} className="text-lg tracking-tight text-white hover:font-semibold">
